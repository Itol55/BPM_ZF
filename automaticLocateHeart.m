function [ cropX, cropY, cropLengthX, cropLengthY, frameHeight, frameWidth ] = automaticLocateHeart( videoFrameCell, findHeartAutomaticallyMethod, heartArea )
    ...
    for ii = 20 : 29
        ...
        heartLocationTabTMP{ ii } = imsharpen( imadjust( rgb2gray( videoFrameCell{ ii } ) ) );
    end
    ...
    for ii = 20 : 29
        for xx = 1 : numberOfAreasToSearch
            for yy = 1 : numberOfAreasToSearch
                croppedFrameFindHeart = imcrop( heartLocationTabTMP{ ii },[ ( ( frameWidth / numberOfAreasToSearch ) * ( yy - 1 ) + 1 ) ( ( frameHeight / numberOfAreasToSearch ) * ( xx - 1 ) + 1 ) ( frameWidth / numberOfAreasToSearch ) ( frameHeight / numberOfAreasToSearch ) ] );
                countIterations = countIterations + 1;
                meanArrayFindHeart( ii - 19, countIterations ) = mean2( croppedFrameFindHeart );
    ...
    for ii = 1 : countGeneralIterations
        for jj = 1 : 10
            averagePixelValueChange( ii ) = meanArrayFindHeart( jj, ii) * ( -1 )^( jj + 1 ) + averagePixelValueChange( ii );
        end
    end
    ...
    for ii = 1 : countGeneralIterations
        if findHeartPixelValue == averagePixelValueChange( ii )
            ...
            positionInArrayX = rem( loopRotationHeartLocation, numberOfAreasToSearch );
            positionInArrayY = ( ( loopRotationHeartLocation / numberOfAreasToSearch ) - ( positionInArrayX / numberOfAreasToSearch ) ) + 1;
        end
    end
    heartCoordinateXX = ( frameWidth / numberOfAreasToSearch ) * ( positionInArrayX - 1 )+ 1 + frameWidth / numberOfAreasToSearch / 2;
    heartCoordinateYY = ( frameHeight / numberOfAreasToSearch )*( positionInArrayY - 1 ) + 1 + frameHeight / numberOfAreasToSearch / 2;
